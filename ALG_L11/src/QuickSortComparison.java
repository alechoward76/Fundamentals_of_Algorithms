...
public class QuickSortComparison {
	...
	private static <E extends Comparable<? super E>> void quickSortIterative(E[] array, int low, int high) {
		...
		stack = new int[high - (low + 1)]; // sets stack size w/o Stack import
		top = -1;// sets top to -1
		push(low);// push low to stack
		push(high);// push high to stack

		do {
			pop();// pops high off
			pop();// pops low off
			int pi = partition(array, low, high);// finds pivot from array
			if (low < (pi - 1)) {
				push(low);
				push(pi - 1);
			}
		...
		} while (top == -1);// perform do loop while top = -1
