...
public class ListReferenceBased<E> implements ListInterface<E>, Iterable<E> {
	...
	/**
	 * copy constructor: create a new list that is a copy of the specified list
	 * 
	 * @param lst: list copied
	 */
	public ListReferenceBased(ListReferenceBased<E> lst) {
	 ...
	 * @param index position of the node containing the item
	 * @return the Object located at the specified index
	 * @throws ListIndexOutOfBoundsException if the index is out of range, i.e. when
	...
	public void add(int index, E item) throws ListIndexOutOfBoundsException {
		if (index >= 1 && index <= numItems + 1) {
			if (index == 1) {
				...
				newNode.setNext(head);  //insert at beginning
				...
				newNode.setNext(prev.getNext());     //inserting after given
				prev.setNext(newNode);
				newNode.setPrev(prev);

				if (newNode.getNext() != null) {  //Change previous node unless null
	 ...
	 * the item is not in the current list. Shifts any subsequent elements to the
	...
	/**
	 * Looks for the index of the specified element in this list. If the element is
	 * not present, the method returns <code>-1</code>
	 * 
	 * @param elt the element which index is looked for.
	...
	// New Method
	public void displayReverse() { //reverses list and displays it

		Node<E> curr = head; //set head as curr
		Node<E> prev = null;

		while (curr.getNext() != null) { //move to end
			curr = curr.getNext();

		}
		head = curr;//move head to tail

		while (curr != null) { //While not null, 
			prev = curr.getPrev(); //set node before curr into a temp placeholder(prev)
			curr.setPrev(curr.getNext()); //set node after curr to before it
			curr.setNext(prev);//take the node stored in prev and place it after curr
			curr = prev;
		}
